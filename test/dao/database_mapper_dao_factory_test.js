...
describe ('DatabaseMappingDaofactory', function () {
    describe ('getInstance', function () {
        it ('throw Error if database type is not supported', function () {
            let factory = new DatabaseMappingDaofactory();
            let settings = new ConnectionSettings(null, null, null, null, 'NotSupportedDatabaseType');
        ...
        it ('return an MSSQLDatabaseMappingDao object when type is MSSQL_2016', function () {
            let factory = new DatabaseMappingDaofactory();
            let settings = new ConnectionSettings(null, null, null, null, DatabaseType.MSSQL_2016);
            let dao = factory.getInstance(settings);
